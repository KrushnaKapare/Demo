...
// Print buffer (kfifo) states in kernel log after every 5 seconds.
// Using Kernel Thread
...
// kernel thread for logging device state
struct task_struct *log_buffer_task;

// periodic task to be done by thread -- display buffer info in log
static int log_buffer_states(void *param)
{
    while (!kthread_should_stop())
    {
        // display info in kernel log
        for (int i = 0; i < devcnt; i++)
        {
            struct kfifo *buf = &devices[i].buffer;
            pr_info("%s: pchar%d buffer => size=%d, len=%d, avail=%d\n",
                    THIS_MODULE->name, i, kfifo_size(buf), kfifo_len(buf), kfifo_avail(buf));
        }
        // repeat after next 5 seconds
        msleep(5000);
    }
    return 0;
}
...
static int __init pchar_init(void)
{
    ...
    // start kernel thread
    log_buffer_task = kthread_run(log_buffer_states, NULL, "buf-loggerd");
    if (IS_ERR(log_buffer_task))
    {
        pr_info("%s: failed to create kernel thread for logging.\n", THIS_MODULE->name);
        goto kthread_run_failed;
    }
    pr_info("%s: kernel thread created for logging device buffer states.\n", THIS_MODULE->name);
...
static void __exit pchar_exit(void)
{
    ...
    // stop kernel thread of buffer state logging timer
    kthread_stop(log_buffer_task);
    pr_info("%s: logging thread is stopped.\n", THIS_MODULE->name);
