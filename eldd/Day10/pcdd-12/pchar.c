...
// timer for display buffer info
struct timer_list timer;

// periodic task to be done by timer -- display buffer info in log
static void log_buffer_states(struct timer_list *ptimer)
{
...
static int __init pchar_init(void)
{
    ...
    // start log timer
    timer_setup(&timer, log_buffer_states, 0);
    timer.expires = jiffies + 2 * HZ; // start after 2 seconds (after insmod)
    add_timer(&timer);
    pr_info("%s: timer is setup for logging device buffer states.\n", THIS_MODULE->name);
...
static void __exit pchar_exit(void)
{
    ...
    // remove buffer state logging timer
    del_timer_sync(&timer);
    pr_info("%s: logging timer is deleted.\n", THIS_MODULE->name);
