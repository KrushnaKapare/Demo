...
static ssize_t pchar_write(struct file *pfile, const char __user *ubuf, size_t ubufsize, loff_t *poffset)
{
    ...
    // check space avail into buffer
    avail_bytes = MAX - *poffset;
    // error check -- if no space left on buffer, return error
    if (avail_bytes == 0)
    {
    ...
    // copy those many bytes and calc number of bytes copied successfully
    nbytes = bytes_to_copy - copy_from_user(buffer + *poffset, ubuf, bytes_to_copy);
    // incr file pos -- based on num of bytes copied
...
static ssize_t pchar_read(struct file *pfile, char __user *ubuf, size_t ubufsize, loff_t *poffset)
{
    ...
    // check avail bytes avail (for reading)
    avail_bytes = MAX - *poffset;
    ...
    bytes_to_copy = avail_bytes < ubufsize ? avail_bytes : ubufsize;
    // copy those many bytes to user space and calc num of bytes copied successfully
    nbytes = bytes_to_copy - copy_to_user(ubuf, buffer + *poffset, bytes_to_copy);
    // increment the file pos accordingly
    *poffset = *poffset + nbytes;
...
{
    loff_t newpos = 0;
    pr_info("%s: pchar_lseek() called with origin=%d, offet=%lld.\n",
            THIS_MODULE->name, origin, offset);
    // calculate newpos based on given offset and origin
    switch (origin)
    {
    case SEEK_SET:
        newpos = 0 + offset;
    ...
    // validate newpos and adjust as required (you may return error as well)
    if (newpos < 0)
        newpos = 0;
    if (newpos > MAX)
        newpos = MAX;
