...
// switch -- bottom half
static void switch_work_handler(struct work_struct *param)
{
    int i;
    // work that cannot be done in top half -- sleeping code also allowed
    // toggle the led 6 times
    for (i = 1; i <= 6; i++)
    {
        led_state = !led_state;
        gpio_set_value(LED_PIN, led_state);
        msleep(500); // sleeping allowed
    }
    pr_info("%s: switch tasklet - led blinked - %d\n", THIS_MODULE->name, led_state);
}
...
static irqreturn_t switch_isr(int irq, void *param)
{
    ...
    // schedule the bottom half -- work queue
...
static int __init pchar_init(void)
{
    ...
    // intialize switch work
    INIT_WORK(&switch_work, switch_work_handler);
...
static void __exit pchar_exit(void)
{
    ...
    // wait for work completion if any
    cancel_work_sync(&switch_work);
    // unregister isr
