...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param)
{
    ...
    switch (cmd)
    {
    case FIFO_CLEAR: // clear the device buffer
        kfifo_reset(&buffer);
        pr_info("%s: ioctl - fifo cleared.\n", THIS_MODULE->name);
        break;
    case FIFO_GET_INFO:
        info.size = kfifo_size(&buffer);
        info.length = kfifo_len(&buffer);
        info.avail = kfifo_avail(&buffer);
        ret = copy_to_user((void *)param, &info, sizeof(struct fifo_info));
        if (ret != 0)
        {
            pr_err("%s: ioctl get info failed.\n", THIS_MODULE->name);
            return -EFAULT;
        }
        pr_info("%s: ioctl - fifo info collected.\n", THIS_MODULE->name);
        break;
    case FIFO_RESIZE:
        // homework
        pr_info("%s: ioctl - fifo is resized.\n", THIS_MODULE->name);
        // 1. alloc a temp buffer using kmalloc() to store current data -- fifo len.
        // 2. get data from fifo into the temp buffer -- kfifo_out()
        // 3. release current fifo -- kfifo_free()
        // 4. re-alloc fifo for new size (3rd param of ioctl) -- kfifo_alloc()
        // 5. add data from temp buf into fifo -- kfifo_in()
        // 6. release temp buffer -- kfree()
        break;
    default:
