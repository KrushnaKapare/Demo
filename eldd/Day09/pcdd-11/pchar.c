...
// Get pid of the current process in pchar_write()/pchar_read().
...
static ssize_t pchar_write(struct file *pfile, const char __user *ubuf, size_t ubufsize, loff_t *poffset)
{
    ...
    int nbytes, ret;
    pr_info("%s: pchar_write() called by process %s pid=%d.\n",
            THIS_MODULE->name, get_current()->comm, get_current()->pid);
