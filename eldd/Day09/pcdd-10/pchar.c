...
// Multiple process should not read/write concurrently into buffer.
// Synchronization to be done using "Semaphore" or "Mutex".
...
typedef struct pchar_device
{
    ...
    // struct semaphore lock; // semaphore for mutual exclusion
...
static int __init pchar_init(void)
{
    ...
    // initialize semaphore/mutex lock
    for (i = 0; i < devcnt; i++)
    {
        // sema_init(&devices[i].lock, 1);
        // pr_info("%s: sema_init() initialized for pchar%d.\n", THIS_MODULE->name, i);
        // mutex_init(&devices[i].lock);
        // pr_info("%s: mutex_init() initialized for pchar%d.\n", THIS_MODULE->name, i);
        spin_lock_init(&devices[i].lock);
        // pr_info("%s: spin_lock_init() initialized for pchar%d.\n", THIS_MODULE->name, i);
    }
...
static ssize_t pchar_write(struct file *pfile, const char __user *ubuf, size_t ubufsize, loff_t *poffset)
{
    ...
    // P(lock);
    // ret = down_interruptible(&dev->lock);
    // Mutex lock
    // ret = mutex_lock_interruptible(&dev->lock);
    // Spinlock lock
    spin_lock(&dev->lock);
    // if wake up due to up(), then ret==0
    // if wake up due to signal, then ret==ERESTARTSYS
    // if (ret != 0)
    // {
    //     pr_info("%s: writer wakeup due to signal.\n", THIS_MODULE->name);
    //     return -ERESTARTSYS;
    // }
...
static ssize_t pchar_read(struct file *pfile, char __user *ubuf, size_t ubufsize, loff_t *poffset)
{
    ...
    // if (ret != 0)
    // {
    //     pr_info("%s: reader wakeup due to signal.\n", THIS_MODULE->name);
    //     return -ERESTARTSYS;
    // }
