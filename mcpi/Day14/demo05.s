...
.type main, %function
main:                              
	// problem: collatz conjure
	//	even --> num / 2, odd --> num * 3 + 1, last term = 1
	//	even --> num>>1, odd --> num<<1 + num + 1
	mov r1, #5					// r1 = 5
loop:
	cmp r1, #1					// if r1 == 1
	beq loop_end				//	goto loop_end
	tst r1, #(1<<0)				// r1 & 1 -- if even, Z=1; else Z=0
	beq even					// beq check if Z=1 (i.e. if res is zero)
//odd:
	mov r2, r1, lsl #1 			// odd: r2 = r1 << 1 (inline barrel shifter)
	add r1, r2					// r1 = r2 + r1
	add r1, #1					// r1 = r1 + 1
	b loop						// repeat loop
even:
	lsr r1, #1					// even: r1 = r1 >> 1 (barrel shifter)
	b loop						// repeat loop
loop_end:
	mov pc, lr
